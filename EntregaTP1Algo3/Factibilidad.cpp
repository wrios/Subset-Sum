...
uint PodaFactibilidad(vector<uint>& elems, uint cant_total_elems, uint ind, uint objetivo, uint cant_en_uso){
	...
	if (objetivo - elems[ind] < 0){return min(sin,cant_total_elems+1);}
	return min(con,sin);
}
...
int main()
{
	...
	uint res = PodaFactibilidad(elems, cant_total_elems, indice, objetivo, cant_en_uso);
